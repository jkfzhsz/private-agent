...
1. **全量测试基线修复**（约 1-2 天）：清理 admin 系列断言缺陷 + 测试隔离（schema 污染）→ 恢复全量回归作为版本门禁。今日已定位具体清单。
2. **KB 版本对比/回滚**（约 1 天）：version_snapshots 表已就绪，补对比与回滚逻辑，价值高（KB 数据安全）。
3. **查询重写**（约 0.5-1 天）：检索前扩展查询词，提升召回精度。
4. **任务级暂停/恢复**（约 1 天）：async_tasks 表就绪，补 DAG 级暂停（会话级已有）。
5. **长期项维持暂缓**：GPU/Docker/交互终端/依赖缓存——内存 7.6GB 与单机约束下收益低。

## 四、结论

- 原定计划：**全部闭环**（含今天收口的 C-4 事件级去重与 context_injected 审计）。
- 新增计划：8-05~8-15 共 11 份，**已实施 10 份、挂起 1 份（B 方案，触发条件明确）**。
- 无"被遗漏的既定计划"；真正待办集中在 **B.1 测试基线质量债**（建议优先）与 **B.2~B.13 的 V2 长期项**（多为明确暂缓）。
