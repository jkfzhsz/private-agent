...
// 2026-08-19(假思考检测): 新增 lastEventAt prop —— 最近一次收到 react_event 的
// 时刻。ThinkingWait 仅在"无 thinking 事件"时显示(等首 token 阶段), 若此期间
// 长时间无任何新事件, 提示"疑似无进展"(模型响应缓慢或后端卡住), 区别于正常思考
// (正常思考会产生 thinking/delta 流式事件, 组件随之卸载)。
export function ThinkingWait({ lastEventAt }: { lastEventAt?: number }): JSX.Element {
  // mountedAt: 组件挂载时刻(等待首 token 的起点); now: 每秒刷新
  ...
  const secs = Math.max(0, Math.round((now - mountedAt) / 1000));
  const idleSecs = lastEventAt
