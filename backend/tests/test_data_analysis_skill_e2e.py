"""M3 plan/m3-skills-data-analysis step 7 - data_analysis skill E2E 测试。

Source: spec/m3-skills-data-analysis AC-1~8
- activate data_analysis → 6 工具过滤(无 web_search/http_request)
- frozen_hash 写入 sessions 表
- system_prompt 含四段式框架关键字
- examples 加载(data_visualization + statistical_test)
...
class TestDataAnalysisSkillE2E:
    """plan step 7: data_analysis skill 完整激活流程 E2E。"""

    def test_activate_data_analysis_filters_tools_and_writes_frozen_hash(self):
        """activate data_analysis → 6 工具(无 web_search/http_request)+ frozen_hash 写入 sessions。"""
        _setup_schema()
    ...
    def test_data_analysis_skill_yaml_matches_blueprint_matrix(self):
        """plan step 1: skill.yaml dependencies.tools 符合蓝图 7.5 矩阵。"""
        async def _run():
            conn = await asyncpg.connect(TEST_DSN)
            try:
                loader = SkillLoader(dev_dir=SKILLS_DEV_DIR)
                skill = await loader.load("data_analysis", conn)
                return skill
            finally:
                await conn.close()
