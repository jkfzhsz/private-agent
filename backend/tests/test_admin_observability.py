...
async def test_usage_aggregation(client, schema):
    ...
    try:
        ...
        await conn.execute(
            ...
            ($1, 1, 'token_usage', '{"model_id":"m1","total_tokens":100,"input_tokens":60,"output_tokens":40,"cached_tokens":30}'::jsonb),
            ($1, 2, 'token_usage', '{"model_id":"m1","total_tokens":200,"input_tokens":120,"output_tokens":80,"cached_tokens":60}'::jsonb)
            """,
            sid,
        )
    finally:
    ...
    assert data["by_session"][0]["cached"] == 90
    assert data["by_session"][0]["cache_hit_rate"] == pytest.approx(0.5, abs=1e-4)
