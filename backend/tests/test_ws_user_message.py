...
def _patch_adapter_and_tools(monkeypatch, responses, tools):
    ...
    # → KeyError → user_message 处理失败 → WS 无 turn_end。
...
def test_user_message_no_tool_calls_produces_thinking_and_final(monkeypatch):
    ...
    # 2026-08-19(A 方案): turn_end 携带服务端权威耗时(ms, monotonic) ——
    # 前端"总耗时"据此显示, 与 turn 号/前端时钟解耦
    assert isinstance(messages[-1].get("duration_ms"), int), (
        f"turn_end 应携带 duration_ms, got {messages[-1]}"
    )
    assert messages[-1]["duration_ms"] >= 0
