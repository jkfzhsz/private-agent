...
@dataclass
class TokenUsage:
    """一次模型调用的 token 用量(2026-08-18 自 billing.py 上移, 供 adapter 解析)。

    cached_tokens: 输入中被缓存命中的 token 数(OpenAI
...
class ChatResult:
    ...
    usage: TokenUsage | None = None  # 2026-08-18: 模型返回 token 用量(计费统计)
