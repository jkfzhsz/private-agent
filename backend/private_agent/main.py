...
# 2026-08-18(故障修复): run_turn 全局超时秒数 —— LLM/内部卡死时中断本轮,
# 释放会话锁(08-18 09:49 故障: run_turn 卡 9+ 分钟无日志, 重连 replay 永久
# 等锁 → "重连不上"; LLM 中转实测正常, 根因在 PA 无轮级超时)。
# 2026-08-18 21:25(全量回归误杀修复): wall-clock 240s 会把正常长任务
# (全量 pytest 回归等持续事件输出/工具执行)误判为卡死中断 —— 改为
# **idle 超时**: 仅当 N 秒内无任何事件(LLM/内部卡死, 09:49 故障场景)
# 才中断本轮。默认 180s, 可用环境变量 PA_RUN_TURN_IDLE_TIMEOUT 调整。
_RUN_TURN_IDLE_TIMEOUT = float(
    os.environ.get("PA_RUN_TURN_IDLE_TIMEOUT", "180")
)
        ...
        while True:
            ...
            if msg_type == "ping":
                ...
                if cancelled_any:
                    ...
                    # 2026-08-18(停止无效反馈): 无运行中任务时也响应 ——
                    # 原实现静默无响应, 前端无任何反馈(用户"按停止键无效").
                    # 典型场景: 历史 interrupted 会话的中断轮(thinking 卡片
                    # 残留但无运行 task), 点停止应明确告知而非无声。
                    await ws.send_json({
                        "type": "turn_cancelled",
                        "session_id": session_id,
                        "message": "当前没有运行中的任务",
                ...
                if not pm.resolve(confirmation_id, approved):
                    # 2026-08-18(请求卡片点不了): error 带 confirmation_id ——
                    # 前端据此标记对应卡片"已过期", 否则用户对失效卡片
                    # 重复点击无任何定位反馈
                    await ws.send_json({
                        "type": "error",
                        "session_id": session_id,
...
async def _handle_user_message(
    ...
    _log_step("enter")
    # 2026-08-18(全量回归误杀修复): 事件进展心跳 —— 所有事件推送
    # (主 loop + delegate_subtask 子代理)更新 last_progress; 外层 run_turn
    # 超时改 idle 语义(仅无进展才中断), 正常长任务不误杀。
    last_progress = time.monotonic()

    async def _ws_event_sink(ev: dict) -> None:
        nonlocal last_progress
        last_progress = time.monotonic()
        await ws.send_json(ev)

    # 2026-08-18(长工具误杀二修): 工具执行期间无事件输出(pytest_run
    # 全量回归等阻塞式长工具), 事件心跳不够 —— ReactLoop 工具执行期
    # 每 10s 调 progress_cb 续期 idle 超时。
    ...
    async with lock:
        try:
            ...
            if resume:
                ...
                # 2026-08-19(蒋先生反馈"断点恢复无反馈"): 断点恢复成功补发
                # turn_resumed(携带 checkpoint_turn) —— 前端明确提示
                # "已从第 N 轮继续", 与流程级"继续"(L973)区分开。
                # 前端据此确认 resume 真正生效, 并触发全量 replay 同步
                # 回滚后的 DB(消除本地旧事件叠加错乱)。
                try:
                    await ws.send_json({
                        "type": "turn_resumed",
            ...
            loop = ReactLoop(
                ...
                event_sink=_ws_event_sink,
                # 2026-08-18(长工具误杀二修): 工具执行期间心跳续期 idle 超时
            ...
            while True:
                ...
                # 2026-08-18(故障修复→误杀修复): run_turn 超时 —— 最初 wall-clock
                # 240s 防卡死占锁(09:49 故障), 但全量 pytest 回归等正常长任务被
                # 误杀(21:25 反馈)。改为 **idle 超时**: create_task 跑 run_turn,
                # 每秒检查距上次事件推送的时间; 仅当 idle > 阈值(默认 180s,
                # 事件 sink 每轮至少推送 thinking/delta/tool 等)才 cancel 中断。
                # 正常长任务(持续事件输出)永不误杀; 真卡死(无任何事件)照旧
                # 中断并释放会话锁(async with 退出), 会话可恢复。
                last_progress = time.monotonic()
                # 2026-08-19(A 方案): 本轮耗时计时起点(服务端权威)。
                # turn_end 携带 duration_ms 下发给前端 —— 与 turn 号/前端
                # 时钟/窗口切换完全解耦(修复: 跨窗口 turn 号撞车 + prev.has
                # 短路导致前端 Date.now() 差值虚高 20 分钟)。
                _turn_started = time.monotonic()
                _turn_task = asyncio.create_task(
                    loop.run_turn(resume=True)
                    if resume
                    else loop.run_turn(run_content)
                )
                while not _turn_task.done():
                    await asyncio.sleep(1)
                    if (
                        time.monotonic() - last_progress
                        > _RUN_TURN_IDLE_TIMEOUT
                    ):
                        _log_step(f"run_turn_idle_timeout_r{rounds}")
                        _turn_task.cancel()
                        break
                try:
                    await _turn_task
                    ...
                    pass
                if _turn_task.cancelled():
                    # idle 超时中断: 推送 error + break(锁随函数退出释放)
                    try:
                        await ws.send_json({
                            "type": "react_event",
                            "event_type": "error",
                            "session_id": session_id,
                            "turn": getattr(loop, "_turn", 0) or 0,
                            "payload": {
                                "message": (
                                    f"【程序异常】本轮长时间无进展"
                                    f"(>{_RUN_TURN_IDLE_TIMEOUT:.0f}s 无事件输出)，"
                                    "已中断释放会话。请重新发送消息重试。"
                                )
                            },
                        })
                ...
                await ws.send_json({
                    ...
                    "session_id": session_id,
                    "turn": loop._turn,
                    # 2026-08-19(A 方案): 服务端权威耗时(monotonic, 毫秒)。
                    # 前端优先用此值显示"总耗时", 废弃 Date.now() 差值
                    # (跨窗口 turn 号撞车/prev.has 短路会污染前端 start)。
                    "duration_ms": int((time.monotonic() - _turn_started) * 1000),
                })
                ...
                if memory_mgr is not None:
                    try:
                        ...
                        # 2026-08-19(蒋先生反馈"任务完成后仍弹 user_message_failed"):
                        # 记忆提取是 turn_end 之后的收尾副作用, 失败不应冒泡到
                        # 外层 except → 误报 user_message_failed(任务已成功完成,
                        # turn_end 已推送)。记日志降级, 不打断流程。
                        _logger.exception(
                            "memory extract failed after turn_end "
                            "(task already completed, session=%s)",
                            session_id,
                        )
