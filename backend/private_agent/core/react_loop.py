...
# 2026-08-18(长工具 idle 误杀): 工具执行期间进展心跳间隔 —— 阻塞式
# 长工具(pytest_run/code_execution/delegate)执行无事件输出, 靠此心跳
# 让 main 侧 run_turn idle 超时续期(不误杀长任务)。
_TOOL_PROGRESS_HEARTBEAT_SEC = 10.0
...
class ReactLoop:
    ...
    def __init__(
        ...
        # 2026-08-18(长工具 idle 误杀): 进展心跳回调 —— 工具执行期间
        # 每 10s 调用一次, main 侧据此续期 run_turn idle 超时。长工具
        # (pytest_run 全量回归/大 code_execution)执行期间无事件输出,
        # 无心跳会被 idle 超时误杀。
                ...
                if self._vision_adapter is not None:
                    ...
                    if chain_has_vision is False:
                        ...
                        if full_chain is not None and full_chain.has_vision:
                            ...
                            return
            try:
                # 2026-08-19(模型调用无感知): LLM 调用前置 status 事件 ——
                ...
                async def _exec_plan(plan: dict) -> ToolResult:
                    async with sem:
                        ...
                        try:
                            ...
                            # 2026-08-18(长工具 idle 误杀): 工具执行期间
                            # 每 10s 触发 progress_cb 心跳 —— main 侧据此
                            # 续期 run_turn idle 超时。pytest_run 全量回归/
                            # 大 code_execution/delegate 等阻塞式长工具执行
                            # 期间无事件推送(事件仅在调用前后), 无心跳会被
                            # idle 超时(180s)误杀中断。
