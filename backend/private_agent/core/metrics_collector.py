    ...
    async def _aggregate_session_metrics(
        ...
        for r in rows:
            ...
            d = agg.setdefault(sid, {
                ...
                "cached_tokens": 0.0, "cache_hit_rate": 0.0,
            })
            et = r["event_type"]
            if et == "final":
                d["turn_count"] += r["cnt"]
        ...
        # token 精确聚合(2026-08-18: 替代 delta/thinking 条数粗估 ——
        # 消费 token_usage 事件的真实用量, 含 cached_tokens/命中率)
        usage_rows = await conn.fetch(
            """
            SELECT session_id, payload FROM react_events
            WHERE event_type = 'token_usage' AND created_at >= $1
            """,
            cutoff,
        )
        for r in usage_rows:
            ...
            d["total_tokens"] += float(p.get("total_tokens") or 0)
            d["input_tokens"] += float(p.get("input_tokens") or 0)
            d["output_tokens"] += float(p.get("output_tokens") or 0)
            d["cached_tokens"] += float(p.get("cached_tokens") or 0)
        for d in agg.values():
            inp = d["input_tokens"]
            d["cache_hit_rate"] = round(d["cached_tokens"] / inp, 4) if inp > 0 else 0.0
        return agg
