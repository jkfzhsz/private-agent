...
async def extract_memory(session_id: int):
    ...
    try:
        ...
        try:
            ...
            # 2026-08-19(修复): partial 绑定 conn —— 与 main.py 一致,
            # 裸传模块函数会缺 conn 参数(TypeError)。
...
async def get_llm_usage(session_id: int | None = None, limit: int = 20):
    """LLM 用量统计(V1.2-6.3, 2026-08-18 去计价): 聚合 react_events 的
    token_usage 事件。

    变更: 移除 total_cost/currency(只统计消耗量, 不计价); 新增
    cached_tokens / cache_hit_rate(cached/input)。
    ...
    Returns:
        200: {total_calls, total_tokens, input_tokens, output_tokens,
              ...
              by_session: [{session_id, calls, tokens, cached_tokens,
                            cache_hit_rate}]}
    """
    import json as _json
        ...
        try:
            ...
            for r in rows:
                ...
                s = per_session.setdefault(
                    sid, {
                        "session_id": sid, "calls": 0, "tokens": 0,
                        "input": 0, "cached": 0, "cache_hit_rate": 0.0,
                    }
                )
                s["calls"] += 1
                s["tokens"] += int(p.get("total_tokens") or 0)
                s["input"] += int(p.get("input_tokens") or 0)
                s["cached"] += int(p.get("cached_tokens") or 0)
                totals["calls"] += 1
                ...
                totals["cached"] += int(p.get("cached_tokens") or 0)
            # 缓存命中率(cached/input, 每会话 + 全局)
            for s in per_session.values():
                inp = s["input"]
                s["cache_hit_rate"] = round(s["cached"] / inp, 4) if inp > 0 else 0.0
            total_cache_rate = (
                round(totals["cached"] / totals["input"], 4)
                if totals["input"] > 0 else 0.0
            )
            by_session = sorted(
                per_session.values(),
            ...
            return {
                ...
                "cache_hit_rate": total_cache_rate,
